...
async function entrarNoSistema() {
  ...
  try {
    ...
    const userRoleReal =
      userData.role || (payload ? payload.role : "CONSULTANT");
    ...
    if (menuAdmin) {
      if (
        userRoleReal === "ADMIN" ||
        userRoleReal === "MANAGER" ||
        userRoleReal === "PC" ||
        userRoleReal === "EXECUTIVO"
      ) {
...
document
  .getElementById("afastamento-form")
  ...
  if (list) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (datas.length === 0) {
          list.innerHTML =
            '<li style="color: var(--success); list-style-type: none;"><i class="ph ph-check"></i> Caminho livre. Nenhum bloqueio iminente.</li>';
        } else {
      ...
      list.innerHTML =
        "<li>Falha ao carregar o radar de bloqueios. Assume risco de rejeição.</li>";
    }
  }
};

document
  .getElementById("voo-diario-form")
  ?.addEventListener("submit", async (e) => {
    e.preventDefault();
    const btn = e.target.querySelector('button[type="submit"]');
...
async function carregarVisaoIndividual() {
  ...
  if (!tbody) return;
  tbody.innerHTML =
    '<tr><td colspan="4" class="text-center dim">Sincronizando seus dados...</td></tr>';
  ...
  try {
    ...
    if (tituloH1 && usuario.nome)
      tituloH1.innerText = `Olá, ${usuario.nome.split(" ")[0]}`;

    const listaTarefas = usuario.tarefas || usuario.tasks || [];

    if (listaTarefas.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="4" class="text-center dim">Nenhuma tarefa atribuída a você no momento.</td></tr>';
    } else {
      tbody.innerHTML = listaTarefas
        .map((t) => {
          const nomeDoProjeto =
            t.projeto_nome || (t.project ? t.project.title : "Projeto Interno");
          ...
          return `
                <tr>
                    ...
                    <td>
                        ${
                          statusDaTarefa !== "CONCLUIDO" &&
                          statusDaTarefa !== "COMPLETED"
...
async function carregarAuditoriaRedbull() {
  ...
  tbody.innerHTML =
    '<tr><td colspan="4" class="text-center dim"><i class="ph ph-spinner ph-spin"></i> Puxando transações do servidor...</td></tr>';
  ...
  try {
    ...
    if (!res.ok) {
      if (res.status === 403)
        throw new Error(
          "Acesso negado. Apenas a Diretoria possui as chaves do cofre.",
        );
    ...
    if (compras.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="4" class="text-center dim">Nenhuma compra de RedBull registrada no sistema.</td></tr>';
      return;
    }
    ...
    compras.forEach((compra) => {
      ...
      const nomeMembro = compra.registered_by
        ? compra.registered_by.name
...
window.abrirModalTarefasMembro = async function (userId, userName) {
  ...
  tbody.innerHTML =
    '<tr><td colspan="4" class="text-center dim"><i class="ph ph-spinner-gap"></i> Extraindo dados da API...</td></tr>';
  ...
  try {
    ...
    if (!res.ok)
      throw new Error("Rota GET /users/{id} não configurada ou sem permissão.");
    ...
    if (listaTarefas.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="4" class="text-center dim">Este membro não possui atribuições operacionais no momento.</td></tr>';
      return;
    }
    ...
    tbody.innerHTML = listaTarefas
      .map((t) => {
        const nomeProj =
          t.projeto_nome ||
          (t.project ? t.project.title : "Avulsa / Sem Projeto");
        ...
        if (t.due_date) {
          ...
          if (
            ...
            corPrazo = "var(--danger)"; // Atrasado
          else if (
            dataPrazo.getTime() === hoje.getTime() &&
            status !== "CONCLUIDO" &&
            status !== "COMPLETED"
          )
            corPrazo = "var(--warning)"; // Vence Hoje
...
async function carregarHistoricoRedBull() {
  ...
  const btnAuditoria = document.getElementById(
    "btn-auditoria-redbull-container",
  );
  ...
  if (!tbody) return;
  tbody.innerHTML =
    '<tr><td colspan="3" class="text-center dim">Consultando a nuvem...</td></tr>';
  ...
  try {
    ...
    if (compras.length === 0) {
      return (tbody.innerHTML =
        '<tr><td colspan="3" class="text-center dim">Você ainda não registrou nenhum consumo.</td></tr>');
    }
    ...
    tbody.innerHTML = compras
      .map((c) => {
        ...
        const horaStr = dataObjeto.toLocaleTimeString("pt-BR", {
          hour: "2-digit",
          minute: "2-digit",
        });
...
async function carregarLeads() {
  try {
    ...
    if (colLead)
      colLead.innerHTML = `<p style="color:var(--danger); font-size:12px; padding:10px;">${error.message}</p>`;
  }
}
...
function renderizarKanban(leads) {
  ...
  const taxaConversao =
    totalLeads > 0 ? Math.round((qtdClientes / totalLeads) * 100) : 0;
  ...
  if (barLead) {
    barLead.style.width = `${pctLead}%`;
    document.getElementById("lbl-lead").innerText = `${Math.round(pctLead)}%`;
  }
  if (barCliente) {
    barCliente.style.width = `${pctCliente}%`;
    document.getElementById("lbl-cliente").innerText =
      `${Math.round(pctCliente)}%`;
  }
  if (barArquivado) {
    barArquivado.style.width = `${pctArquivado}%`;
    document.getElementById("lbl-arquivado").innerText =
      `${Math.round(pctArquivado)}%`;
  }

  // 5. PREENCHIMENTO DOS CARDS
  if (leads.length === 0) {
    document.getElementById("cards-LEAD").innerHTML =
      '<p class="dim text-center" style="padding:15px; font-size:13px;">Nenhum lead no banco.</p>';
    return;
  }
  ...
  leads.forEach((lead) => {
    ...
    const cnpjFormatado = lead.cnpj.replace(
      /^(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/,
      "$1.$2.$3/$4-$5",
    );
    ...
    const container =
      document.getElementById(`cards-${lead.status}`) ||
      document.getElementById("cards-LEAD");
...
// ==========================================
// COMUNICAÇÃO INTEGRADA (QUADRO DE AVISOS)
// ==========================================
let mapaUsuariosCache = null;
let intervalMensagens = null;

async function obterMapaUsuarios(forcar = false) {
  if (mapaUsuariosCache && !forcar) return mapaUsuariosCache;
  try {
    ...
    mapaUsuariosCache = {};
    usuarios.forEach((u) => {
      mapaUsuariosCache[u.id] = u.name || u.nome || `Usuário #${u.id}`;
    });
  } catch (e) {
    mapaUsuariosCache = mapaUsuariosCache || {};
  }
  return mapaUsuariosCache;
}

function escaparHtmlAviso(str) {
  const div = document.createElement("div");
  div.innerText = str ?? "";
  return div.innerHTML;
}

function formatarDataHoraAviso(isoString) {
  ...
  return ehHoje
    ? `Hoje às ${horaStr}`
    : `${data.toLocaleDateString("pt-BR")} às ${horaStr}`;
}

async function carregarMensagens(scrollToBottom = true) {
  const container = document.getElementById("mensagens-list");
  if (!container) return;

  const primeiraCarga = container.dataset.loaded !== "true";
  if (primeiraCarga) {
    container.innerHTML =
      '<p class="dim text-center" style="padding: 30px 0;">Sincronizando avisos...</p>';
  }

  try {
    const [mapaUsuarios, res] = await Promise.all([
      obterMapaUsuarios(),
      fetchSeguro("/messages/?limit=20&offset=0"),
    ]);

    if (!res.ok)
      throw new Error("Não foi possível carregar o quadro de avisos.");
    ...
    if (!mensagens || mensagens.length === 0) {
      container.innerHTML =
        '<p class="dim text-center" style="padding: 30px 0;">Nenhum aviso publicado ainda. Seja o primeiro a comunicar algo à equipe!</p>';
      return;
    }

    // A API não garante ordem; ordenamos sempre do mais antigo para o mais novo (estilo chat)
    ...
    container.innerHTML = ordenadas
      .map((msg) => {
        const autorNome =
          mapaUsuarios[msg.user_id] || `Usuário #${msg.user_id}`;
        const ehMeu = msg.user_id === currentUserId;
        return `
                <div class="msg-item ${ehMeu ? "msg-item-own" : ""}">
                    <div class="msg-item-header">
                        <span class="msg-item-author">${escaparHtmlAviso(autorNome)}${ehMeu ? " (você)" : ""}</span>
                        <span class="msg-item-time">${formatarDataHoraAviso(msg.created_at)}</span>
                    </div>
                    <div class="msg-item-content">${escaparHtmlAviso(msg.content)}</div>
                </div>
            `;
      })
      .join("");
    ...
    if (primeiraCarga) {
      container.innerHTML = `<p class="dim text-center" style="padding: 30px 0; color: var(--danger);">${error.message}</p>`;
    }
    console.error("Erro ao carregar avisos:", error);
  }
}

function iniciarAutoRefreshAvisos() {
  pararAutoRefreshAvisos();
  intervalMensagens = setInterval(() => carregarMensagens(false), 20000);
}

function pararAutoRefreshAvisos() {
...
document.getElementById("msg-input")?.addEventListener("input", function () {
  ...
  if (counter) counter.innerText = `${this.value.length}/1500`;
...
document.getElementById("msg-input")?.addEventListener("keydown", function (e) {
  if (e.key === "Enter" && !e.shiftKey) {
    e.preventDefault();
    document.getElementById("form-quadro-avisos")?.requestSubmit();
  }
});

document
  .getElementById("btn-refresh-avisos")
  ?.addEventListener("click", () => carregarMensagens(true));

document
  .getElementById("form-quadro-avisos")
  ?.addEventListener("submit", async (e) => {
    e.preventDefault();
    ...
    if (conteudo.length > 1500) {
      alert("O aviso excede o limite de 1500 caracteres.");
      return;
    }

    const originalHtml = btn.innerHTML;
    btn.disabled = true;
    btn.innerHTML = '<i class="ph ph-circle-notch"></i> Publicando...';

    try {
      const res = await fetchSeguro("/messages/", {
        method: "POST",
      ...
      if (res.status === 429) {
        throw new Error(
          "Limite de 3 publicações por minuto atingido. Aguarde um instante e tente novamente.",
        );
      }
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        let msg = err.detail || "Erro ao publicar o aviso.";
      ...
      const counter = document.getElementById("msg-char-count");
      if (counter) counter.innerText = "0/1500";
...
async function carregarProjetosAcompanhamento() {
  ...
  if (!listBody) return;
  listBody.innerHTML =
    '<tr><td colspan="4" class="text-center dim">Buscando projetos...</td></tr>';
  try {
    ...
    if (projetos.length === 0)
      return (listBody.innerHTML =
        '<tr><td colspan="4" class="text-center dim">Nenhum projeto ativo.</td></tr>');

    listBody.innerHTML = projetos
      .map(
        (p) => `
            <tr>
...
document
  ...
  ?.addEventListener("submit", async (e) => {
    ...
    if (
      !confirm(
        "Isso irá gerar bloqueios automáticos a cada 15 dias. Confirma a operação?",
      )
    )
      return;
...
async function carregarLeadsParaPrecificacao() {
  ...
  if (!selector) return;
  selector.innerHTML =
    '<option value="">Buscando organizações do banco...</option>';
  try {
    ...
    if (leads.length === 0)
      return (selector.innerHTML =
        '<option value="" disabled selected>Nenhum lead/cliente encontrado.</option>');
    selector.innerHTML =
      '<option value="" disabled selected>-- Selecione o Lead/Cliente --</option>' +
      leads.map((l) => `<option value="${l.id}">${l.name}</option>`).join("");
  } catch (err) {
    selector.innerHTML =
      '<option value="" disabled>Erro ao carregar organizações</option>';
  }
}
...
async function carregarComplianceAdmin() {
  ...
  try {
    ...
    if (flags.length === 0)
      return (tbodyFlags.innerHTML =
        '<tr><td colspan="3" class="text-center dim">Nenhuma punição registrada na EJ.</td></tr>');

    tbodyFlags.innerHTML = flags
      .map((f) => {
        const nomeMembro = f.user
          ? f.user.name || f.user.nome
          : `ID: ${f.user_id}`;
        const isFormal = f.severity === "FORMAL";
...
async function carregarTodasFaltasAdmin() {
  ...
  try {
    ...
    if (faltas.length === 0)
      return (tbodyFaltas.innerHTML =
        '<tr><td colspan="4" class="text-center dim">Nenhuma ausência reportada na empresa.</td></tr>');

    tbodyFaltas.innerHTML = faltas
      .map((f) => {
        const nomeMembro = f.user
          ? f.user.name || f.user.nome
          : `ID: ${f.user_id}`;
        const dataCorrigida = new Date(
          f.absence_date + "T12:00:00",
        ).toLocaleDateString("pt-BR");
        const statusClass =
          f.status === "APROVADA"
            ? "status-approved"
...
async function carregarFaltas() {
  ...
  try {
    ...
    if (faltas.length === 0)
      return (tbody.innerHTML =
        '<tr><td colspan="3" class="text-center dim">Nenhum registro de falta no histórico.</td></tr>');

    tbody.innerHTML = faltas
      .map((f) => {
        const dataCorrigida = new Date(
          f.absence_date + "T12:00:00",
        ).toLocaleDateString("pt-BR");
...
window.prepararNovoProjeto = async function () {
  ...
  document.querySelector("#modal-projeto .modal-header h3").innerText =
    "Criar Novo Projeto";
  document.querySelector('#projeto-form button[type="submit"]').innerText =
    "Salvar Projeto";
...
window.delegarTarefa = async function () {
  ...
  if (!projectId)
    return alert("Erro: O projeto precisa ser salvo antes de delegar tarefas.");
  ...
  try {
    ...
    if (!res.ok)
      throw new Error((await res.json()).detail || "Erro ao delegar.");
    ...
    if (projeto.members && Array.isArray(projeto.members)) {
      projeto.members.forEach((membro) => {
        const cb = document.querySelector(
          `.member-checkbox[value="${membro.id}"]`,
        );
        if (cb) cb.checked = true;
      });
    }

    const delegationSection = document.getElementById(
...
async function carregarLeadsParaProjetos() {
  ...
  try {
    ...
    if (leads.length === 0)
      return (selector.innerHTML =
        '<option value=\"\" disabled selected>Nenhum cliente cadastrado.</option>');
    selector.innerHTML =
  ...
  try {
    ...
    selector.innerHTML =
      ...
      projetos
        .map(
          (p) =>
            `<option value="${p.id}">${p.title || p.name || "Projeto ID: " + p.id}</option>`,
        )
        .join("");
  } catch (err) {
...
async function carregarTabelaPonto() {
  try {
    ...
    if (badgeStatus) {
      if (dados.is_working) {
        badgeStatus.innerHTML =
          '<span style="color: var(--warning);"><i class="ph ph-clock"></i> TRABALHANDO AGORA</span>';
        badgeStatus.style.borderColor = "var(--warning)";
      } else {
        badgeStatus.innerHTML =
          '<span class="dim"><i class="ph ph-coffee"></i> FORA DO EXPEDIENTE</span>';
    ...
    atualizarVisualizacaoPonto(
      dados.worked_minutes_today,
      dados.worked_minutes_this_week,
    );
  } catch (e) {
    const painelHoras = document.getElementById("ponto-total-horas");
    if (painelHoras)
      painelHoras.innerHTML = `<span style="font-size: 20px; color: var(--danger);">${e.message}</span>`;
  }
}
...
async function carregarTabelaReembolsos() {
  ...
  if (isDirex) {
    ...
    thRow.innerHTML =
      "<th>Membro/Título</th><th>Categoria</th><th>Valor</th><th>Ações</th>";
  } else {
    thRow.innerHTML =
      "<th>Data/Título</th><th>Categoria</th><th>Valor</th><th>Status / Ações</th>";
  }

  tbody.innerHTML =
    '<tr><td colspan="4" class="text-center dim">Sincronizando caixa...</td></tr>';
  ...
  try {
    ...
    tbody.innerHTML = dados
      .map((item) => {
        ...
        const valor = parseFloat(item.value).toLocaleString("pt-BR", {
          style: "currency",
          currency: "BRL",
        });
        ...
        const identificador = isDirex
          ? item.user?.name || `Membro #${item.user_id}`
          : item.title;
...
async function carregarDadosTime() {
  ...
  if (!listBody) return;
  listBody.innerHTML =
    '<tr><td colspan="4" class="text-center dim">Sincronizando time...</td></tr>';
  ...
  try {
    ...
    if (listaMembros.length > 0) {
      ...
      listBody.innerHTML = listaMembros
        .map((m) => {
          ...
          const atividade =
            projetosAtivos > 0
              ? `${projetosAtivos} Projeto(s) Ativo(s)`
              : "Livre / Disponível";
          const statusColor =
            projetosAtivos > 0 ? "status-ativo" : "status-away";
          ...
          let turnoHtml =
            '<span class="dim small"><i class="ph ph-coffee"></i> Offline</span>';
      ...
      const membrosAlocados = listaMembros.filter(
        (m) => (m.active_projects_count || 0) > 0,
      ).length;
      const porcentagemGeral =
        Math.round((membrosAlocados / totalMembros) * 100) || 0;
      const livresCount = listaMembros.filter(
        (m) => (m.active_projects_count || 0) === 0,
      ).length;
      const gargaloCount = listaMembros.filter(
        (m) => (m.active_projects_count || 0) >= 3,
      ).length;
      const capacidadeTotal = totalMembros * 20;
      const horasConsumidas = listaMembros.reduce((acc, item) => {
        const m = item.user || item;
        const horasMembro =
          parseFloat(m.weekly_hours) || (m.active_projects_count || 0) * 10;
        return acc + horasMembro;
      }, 0);
      const porcentagemHoras =
        Math.min(Math.round((horasConsumidas / capacidadeTotal) * 100), 100) ||
        0;
      ...
      if (textGargalo) {
        ...
        textGargalo.style.color =
          gargaloCount > 0 ? "var(--danger)" : "var(--success)";
      }
      ...
      if (textHoras) {
        ...
        textHoras.style.color =
          horasConsumidas > capacidadeTotal
            ? "var(--danger)"
            : "var(--text-main)";
      }
      if (barraHoras) {
        barraHoras.style.width = `${porcentagemHoras}%`;
        barraHoras.style.background =
          porcentagemHoras >= 90
            ? "var(--danger)"
            : porcentagemHoras >= 70
              ? "var(--warning)"
      ...
      listBody.innerHTML =
        '<tr><td colspan="4" class="text-center dim">Nenhum membro ativo retornado.</td></tr>';
      const chart = document.getElementById("team-occupancy-chart");
      if (chart) {
...
async function carregarUsuariosAdmin() {
  ...
  if (!tbody) return;
  tbody.innerHTML =
    '<tr><td colspan="5" class="text-center dim">Sincronizando banco de dados...</td></tr>';
  ...
  try {
    ...
    if (usuarios.length === 0)
      return (tbody.innerHTML =
        '<tr><td colspan="5" class="text-center dim">Nenhum membro encontrado.</td></tr>');
...
window.prepararNovoUsuario = function () {
  ...
  document.getElementById("modal-usuario-title").innerText =
...
window.deletarUsuario = async function (userId, userEmail) {
  if (
    !confirm(
      `TOLERÂNCIA ZERO: Você está prestes a DELETAR a conta de ${userEmail}. Confirma a exclusão física?`,
    )
  )
    return;
  try {
    ...
    if (!res.ok)
      throw new Error(
        (await res.json()).detail || "Erro ao deletar o usuário no banco.",
      );
...
async function carregarCheckboxesDeMembros() {
  ...
  try {
    ...
    container.innerHTML =
      '<span style="color: var(--danger)">Falha ao carregar a lista de membros.</span>';
  }
}
...
async function carregarAfastamentos() {
  ...
  const userRole = localStorage.getItem("userRole");
  const isDirex =
    userRole === "ADMIN" || userRole === "EXECUTIVO" || userRole === "PC";
  ...
  if (isDirex) {
    ...
    if (widget && listContainer) {
      ...
      fetchSeguro("/flight-mode/today")
        .then(async (res) => {
          if (res.ok) {
            const foraHoje = await res.json();
            if (foraHoje.length === 0) {
              listContainer.innerHTML =
                '<span class="status-badge status-approved"><i class="ph ph-check-circle"></i> Força Total (100% Operacional)</span>';
            } else {
              listContainer.innerHTML = foraHoje
                .map(
                  (p) => `
  ...
  tbody.innerHTML =
    '<tr><td colspan="4" class="text-center dim">Sincronizando solicitações com a API...</td></tr>';

  try {
    const endpoint = isDirex
      ? "/leave-requests/all"
      : "/flight-mode/my-calendar";
    const res = await fetchSeguro(endpoint);
    ...
    if (dados.length === 0) {
      return (tbody.innerHTML =
        '<tr><td colspan="4" class="text-center dim">Nenhuma solicitação encontrada.</td></tr>');
    }
    ...
    tbody.innerHTML = dados
      .map((req) => {
        ...
        const startDate = new Date(
          req.start_date + "T12:00:00",
        ).toLocaleDateString("pt-BR");
        const endDate = new Date(req.end_date + "T12:00:00").toLocaleDateString(
          "pt-BR",
        );
        ...
        const identificador = isDirex
          ? req.user?.name || req.user?.nome || `Membro #${req.user_id}`
          : `Sua Solicitação`;
...
window.julgarAfastamento = async function (id, novoStatus) {
  if (
    !confirm(
      `Atenção: Você tem certeza que deseja ${novoStatus} esta solicitação de afastamento?`,
    )
  )
    return;
  try {
    ...
    if (!res.ok)
      throw new Error(
        (await res.json()).detail || "Erro de permissão ou status inválido.",
      );
...
function renderizarKanbanParceiros(parceiros) {
  ...
  parceiros.forEach((p) => {
    ...
    let tempColor =
      temp === "QUENTE"
        ? "color: var(--danger); background: rgba(239, 68, 68, 0.1); border: 1px solid var(--danger);"
        : "color: #0ea5e9; background: rgba(14, 165, 233, 0.1); border: 1px solid #0ea5e9;";
...
window.prepararNovoParceiro = function () {
  ...
  document.getElementById("modal-partner-title").innerText =
...
window.abrirEdicaoParceiro = function (id) {
  ...
  if (!p)
    return alert("Erro Crítico: Parceiro não encontrado na memória local.");
...
document
  .getElementById("form-prm-operacional")
  ?.addEventListener("submit", async (e) => {
    e.preventDefault();
...
document
  ...
  ?.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (!res.ok)
        throw new Error("Erro de comunicação com o servidor Backend.");
...
document.getElementById("flag-form")?.addEventListener("submit", async (e) => {
  ...
  try {
    const res = await fetchSeguro(`/users/${targetId}/flags`, {
      method: "POST",
      body: JSON.stringify(payload),
    });
    if (!res.ok)
      throw new Error((await res.json()).detail || "Erro de permissão.");
    ...
    btnSubmit.innerText = "Registrar Punição";
    btnSubmit.disabled = false;
  }
});
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .getElementById("btn-pdf-orcamento")
    ?.addEventListener("click", async (e) => {
      ...
      if (!leadId)
        return alert(
          "Erro: Selecione a organização/lead no menu superior antes de gerar a proposta comercial.",
        );
      ...
      const rateio =
        parseFloat(document.getElementById("preco-rateio").value) || 0;
      const margemDecimal =
        (parseFloat(document.getElementById("preco-margem").value) || 0) / 100;
      const impostoDecimal =
        (parseFloat(document.getElementById("preco-imposto").value) || 0) / 100;
      ...
      try {
        ...
        if (!res.ok)
          throw new Error(
            (await res.json()).detail || "Falha na geração do documento.",
          );
  ...
  document
    .getElementById("btn-calcular-preco")
    ?.addEventListener("click", async (e) => {
      const btn = e.currentTarget;
      ...
      try {
        ...
        if (!res.ok)
          throw new Error(
            (await res.json()).detail ||
              "Erro matemático na formatação dos dados.",
          );
        ...
        document.getElementById("res-custo").innerText = formatarMoeda(
          resultado.total_direct_cost,
        );
        document.getElementById("res-imposto").innerText = formatarMoeda(
          resultado.tax_value,
        );
        document.getElementById("res-lucro").innerText = formatarMoeda(
          resultado.margin_value,
        );
        document.getElementById("res-venda").innerText = formatarMoeda(
          resultado.final_project_value,
        );
      } catch (err) {
  ...
  document.getElementById("file-reembolso")?.addEventListener("change", (e) => {
    document.getElementById("file-name-display").innerText =
      e.target.files[0]?.name || "Arraste ou clique";
  });

  document.getElementById("file-redbull")?.addEventListener("change", (e) => {
    const fileName =
      e.target.files[0]?.name || "Clique ou arraste o arquivo aqui";
    document.getElementById("file-name-rb").innerText = fileName;
  });

  document
    .getElementById("redbull-form")
    ?.addEventListener("submit", async (e) => {
      e.preventDefault();
      ...
      try {
        ...
        e.target.reset();
        document.getElementById("file-name-rb").innerText =
  ...
  document
    .getElementById("btn-bater-ponto")
    ?.addEventListener("click", async (e) => {
      const btn = e.currentTarget;
      btn.innerHTML = '<i class="ph ph-spinner-gap"></i> Registrando...';
      btn.disabled = true;
      try {
        const res = await fetchSeguro("/clockins/register", {
          method: "POST",
          body: JSON.stringify({}),
        });
        ...
        alert("Falha na API de Ponto: " + err.message);
      } finally {
        btn.innerHTML =
          '<i class="ph ph-fingerprint"></i> Registrar Entrada / Saída';
        btn.disabled = false;
      }
    });
  ...
  document
    ...
    ?.addEventListener("submit", async (e) => {
      ...
      const payload = {
        ...
        organization_id:
          parseInt(document.getElementById("proj-client").value) || null,
      ...
      try {
        let res;
        if (editId) {
          res = await fetchSeguro(`/projects/${editId}/editar`, {
            method: "PATCH",
            body: JSON.stringify(payload),
          });
        } else {
          res = await fetchSeguro("/projects/", {
            method: "POST",
            body: JSON.stringify(payload),
          });
        }
        ...
        if (!res.ok)
          throw new Error((await res.json()).detail || "Erro de Validação");
  ...
  document
    ...
    ?.addEventListener("submit", async (e) => {
      ...
      const leadData = {
        ...
        status: "LEAD",
        contacts: Array.from(document.querySelectorAll(".contact-row")).map(
          (r) => ({
  ...
  document
    ...
    ?.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        const resBanco = await fetchSeguro("/reimbursements/", {
          method: "POST",
          body: JSON.stringify(dadosReembolso),
        });
        if (!resBanco.ok)
          throw new Error("Erro ao salvar no banco financeiro.");
  ...
  document
    .getElementById("btn-calculate-diag")
    ?.addEventListener("click", async (e) => {
      const btn = e.currentTarget;
      const projectId = document.getElementById("diag-project-selector").value;
      if (!projectId)
        return alert(
          "ERRO DE OPERAÇÃO: Selecione um projeto na lista acima antes de calcular.",
        );
      ...
      document.querySelectorAll("#task-body tr").forEach((row) => {
        ...
        if (id) {
          payload.tasks[id] = {
            ...
            pred:
              inputs[2].value.trim() === "-" || inputs[2].value.trim() === ""
                ? []
                : inputs[2].value
                    .trim()
                    .toUpperCase()
      ...
      if (Object.keys(payload.tasks).length === 0)
        return alert("Adicione tarefas na matriz.");
      btn.innerHTML = '<i class="ph ph-spinner-gap"></i> ...';
      btn.disabled = true;

      try {
        const res = await fetchSeguro(`/projects/${projectId}/diagnostic`, {
          method: "PATCH",
          body: JSON.stringify(payload),
        });
        ...
        if (!res.ok)
          throw new Error(jsonResponse.detail || "Erro interno na API");
        ...
        document.getElementById("pert-total").innerText =
          `${pert.tempo_enxuto_horas} h`;
        document.getElementById("pert-safety").innerText =
          `${pert.margem_seguranca_horas} h`;
        document.getElementById("critical-path-text").innerText =
          dadosDiagnostico.pert_classico.caminho_critico.join(" -> ");

        document.getElementById("ccpm-meta").innerText =
          `${ccpm.tempo_agressivo_projeto_horas} h`;
        document.getElementById("ccpm-buffer").innerText =
          `${ccpm.project_buffer_horas} h`;
        document.getElementById("buffer-status").innerText =
          "Protegido Matematicamente";
      } catch (err) {
  ...
  document
    ...
    ?.addEventListener("click", async () => {
      ...
      try {
        ...
        if (!res.ok)
          throw new Error("Gere o cálculo na tela antes de exportar.");
        const url = window.URL.createObjectURL(await res.blob());
        const a = document.createElement("a");
        a.href = url;
        a.download = `Diagnostico_Projeto_${projectId}.pdf`;
        document.body.appendChild(a);
        a.click();
        a.remove();
      } catch (err) {
  ...
  document
    ...
    ?.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (!res.ok)
          throw new Error("Erro ao registrar a falta no banco de dados.");
        ...
        btnSubmit.innerText = "Enviar Justificativa";
        btnSubmit.disabled = false;
      }
    });
});
...
window.revogarBandeira = async function (flagId) {
  if (
    !confirm(
      "Tem certeza que deseja REVOGAR esta bandeira? Esta ação apagará o registro histórico.",
    )
  )
    return;
  try {
    ...
    if (!res.ok)
      throw new Error((await res.json()).detail || "Erro ao revogar bandeira.");
...
document.getElementById("cargo-form")?.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (!res.ok)
      throw new Error((await res.json()).detail || "Falha ao alterar cargo.");
  ...
  try {
    ...
    if (projetos.length === 0) {
      selectProjetos.innerHTML =
        '<option value="" disabled>Nenhum projeto cadastrado no sistema.</option>';
    } else {
      selectProjetos.innerHTML =
        '<option value="" disabled selected>-- Selecione um Projeto --</option>' +
        projetos
          .map((p) => `<option value="${p.id}">${p.title || p.name}</option>`)
          .join("");
    }
  } catch (err) {
    selectProjetos.innerHTML =
      '<option value="" disabled>Erro ao carregar projetos.</option>';
  }
};

document
  .getElementById("delegacao-rapida-form")
  ?.addEventListener("submit", async (e) => {
    e.preventDefault();
    const btn = e.target.querySelector('button[type="submit"]');
    ...
    const title = document
      .getElementById("delegacao-tarefa-titulo")
      .value.trim();
    ...
    try {
      ...
      if (!res.ok)
        throw new Error(
          (await res.json()).detail || "Falha ao delegar a tarefa.",
        );
...
document.getElementById("senha-form")?.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (!res.ok)
      throw new Error(
        (await res.json()).detail ||
          "Senha atual incorreta ou erro no servidor.",
      );
...
async function exportarComprovantesRedbull() {
  try {
    ...
    let csvContent =
      "Membro;Quantidade;Data_Registro;Status_Pagamento;Link_Comprovante\n";
    ...
    linkDownload.setAttribute(
      "download",
      `Auditoria_RedBull_${new Date().getTime()}.csv`,
    );
...
document
  .querySelector('[data-target="partners"]')
